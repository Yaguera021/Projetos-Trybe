...
export const sumProductsPrice = async () => {
  const getTotalPriceEl = document.querySelector('.total-price');
  const arrayId = getSavedCartIDs();
  const infoProd = await Promise.all(arrayId.map(async (item) => fetchProduct(item)));
  const sumValues = infoProd.reduce((acc, cur) => (acc + cur.price), 0);
  getTotalPriceEl.innerText = (sumValues).toFixed(2);
};
...
export const addToCart = async (id) => {
  ...
  const createdProduct = createCartProductElement(productFetched);
  const getSec = document.querySelector('.cart__products');
  getSec.appendChild(createdProduct);
  sumProductsPrice();
};
